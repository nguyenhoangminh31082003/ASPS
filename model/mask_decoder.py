...
from .transformer import Attention, TwoWayTransformer
...
class features_feat(nn.Module):
    ...
    def forward(
        ...
        cnn_features_feat = F.interpolate(
            cnn_features_feat, size=hq_features.shape[2:], mode="bilinear"
        )
...
class MaskDecoder(nn.Module):
    def __init__(
        ...
        self.cnn_modify = nn.Sequential(
            nn.Conv2d(cnn_dim, transformer_dim, 3, 1, 1), LayerNorm2d(transformer_dim), nn.GELU()
        )
    ...
    def predict_masks(
        ...
        hs, src = self.transformer(
            src, pos_src, tokens, cnn_k
        )  # queries, keys # torch.Size([1, 15, 256]) torch.Size([1, 4096, 256])
        iou_token_out = hs[:, 0, :]
        ...
        upscaled_embedding = (
            self.embedding_maskfeature(upscaled_embedding_sam) + fusion_feature
        )  # !!!!!
