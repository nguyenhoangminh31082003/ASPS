...
class ASPS(nn.Module):
    def __init__(self, vit_model, vit_pretrained, cnn_model, cnn_pretrained, cnn_image_size):
        ...
        self.network = TrainedASPSNetwork(vit_model, cnn_model, cnn_pretrained, cnn_image_size)

    def forward(self, input, multimask_output=False):
        image_embeddings, interm_embeddings = self.vit(input)

        masks, iou_pred, uncertainty_p = self.network(
            input, image_embeddings, interm_embeddings, multimask_output
        )

        return masks, iou_pred, uncertainty_p


class TrainedASPSNetwork(nn.Module):
    def __init__(self, vit_model, cnn_model, cnn_pretrained, cnn_image_size):
        super().__init__()
    ...
    def forward(self, input, image_embeddings, interm_embeddings, multimask_output):
